...
public record Cluster
{
    ...
    public IntraEdge GetIntraEdge(Vector2 startPoint, Direction direction)
    {
        for (var i = 0; i < IntraEdges.Count; i++)
        {
            ...
            if (edge.Path[0] == startPoint &&
                edge.AdjacencyDirection ==
                direction) //Need point equality or to move away from using points, probably this one.
                return edge;
        }
    ...
    public Vector2[]
        FindConcretePath(Vector2 startPoint, Vector2 endPoint) //This is A* as normal within the confines of a cluster.
    {
        ...
        while (_sortedOpenConcreteNodes.Count > 0)
        {
            ...
            for (var index = 0; index < 4; index++)
            {
                ...
                if (!currentNode.Cluster.IsInValidPosition(neighbour)) continue;

                var node = new ConcreteNode(currentNode.Cluster, currentNode, endNode, neighbour,
                    currentNode.Cost +
                    1); //Back to hardcoding this for now because the og neighbour finder stuff is ugly.

                if (_closedConcreteNodes
                    .Contains(node.Position)) //Don't need to recheck node if it's already be looked at
                    continue;

                if (_openConcreteNodes.TryGetValue(node.Position, out var existingOpenNode))
                {
                    if (!(existingOpenNode.CostDistance > node.CostDistance)) continue;
    ...
    public Cluster(int column, int row, Vector2 clusterSize)
    {
        ...
        var rectSize =
            clusterSize -
            new Vector2(1,
                1); //We want the rectangle to contain as many points as the cluster size along its edge hence -1
        Region = BoundingRectangle.FromPositionAndSize(new Vector2(column * clusterSize.X, row * clusterSize.Y),
            rectSize);
    ...
    public Cluster(Vector2 clusterSize)
    {
        ...
        DoorPoints = new List<EdgePoint>
        {
            EdgePoints[0], EdgePoints[9], EdgePoints[10], EdgePoints[19], EdgePoints[20], EdgePoints[29],
            EdgePoints[30], EdgePoints[39]
        };
        IntraEdges = new List<IntraEdge>();
    }
