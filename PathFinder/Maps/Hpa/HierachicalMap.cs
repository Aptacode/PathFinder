...
public class HierachicalMap : IDisposable
{
    ...
    public HierachicalMap(Scene scene)
    {
        ...
        var clusterColumnCount =
            (int)(scene.Size.X / clusterSize.X); //Map dimensions must be divisible by chosen cluster size
        ...
        for (var x = 0; x < clusterColumnCount; x++)
        {
            ...
            for (var y = 0; y < clusterRowCount; y++) clusters[x][y] = new Cluster(x, y, clusterSize);
        }

        _clusters = clusters;
        UpdateClusters();

        foreach (var componentViewModel in scene.Components) Add(componentViewModel);
    ...
    public void UpdateDoorPoints(Cluster cluster, Cluster adjacentCluster, Direction direction, int clusterWidth,
        int clusterHeight)
    {
        if (adjacentCluster == Cluster.Empty) return;
        ...
        for (var i = 0; i < edgeCount; i++)
        {
            ...
            else if (j != 0)
            {
                ...
                if (j > 1) cluster.DoorPoints.Add(cluster.EdgePoints[offset + i - j]);
        ...
        if (j > 1) cluster.DoorPoints.Add(cluster.EdgePoints[offset + edgeCount - j]);
    }
    ...
    {
        if (abstractPath.Length == 0) return Array.Empty<Vector2>();
        ...
        for (var i = 1; i < abstractPath.Length; i++) //The first node does not have a parentIntraEdge of relevance
        {
            ...
            for (var j = 0; j < path.Length; j++) output[index++] = path[j];
        }
    ...
    public void CheckCluster(Cluster currentCluster, Cluster adjacentCluster, AbstractNode currentNode,
        Direction direction, AbstractNode endNode)
    {
        var intraEdge = currentCluster.GetIntraEdge(currentNode.DoorPoint, direction);
        if (intraEdge.Equals(IntraEdge.Empty)) return;

        var neighbourDoorPoint = intraEdge.Path.Last().GetAdjacentPoint(intraEdge.AdjacencyDirection);
        var neighbourCost =
            currentNode.Cost +
            intraEdge.Path
                .Length; //This is 1 more than the actual path length but works for our purposes so we don't need to add inter-edge costs (1)
        neighbourCost += currentNode.ParentIntraEdge.AdjacencyDirection == direction ? -1 : 0;
        var node = new AbstractNode(currentNode, endNode, adjacentCluster, neighbourDoorPoint, intraEdge,
            neighbourCost);
        ...
        {
            if (!(existingNode.CostDistance > node.CostDistance)) return;
    ...
    public AbstractNode[]
        FindAbstractPath(Vector2 startPoint, Vector2 endPoint) //This is A* on Hierachical map clusters as nodes
    {
    ...
    public void Update(ComponentViewModel component)
    {
        ...
        for (var j = 0; j < _clusters[i].Length; j++)
        {
            ...
            if (!component.CollidesWith(cluster.Region)) continue;
        ...
        foreach (var invalidatedCluster in _invalidatedClusters) UpdateCluster(invalidatedCluster);
    }
    ...
    {
        if (x >= 0 && x < _clusterColumnCount && y >= 0 && y < _clusterRowCount) return _clusters[x][y];
    ...
    public void UpdateIntraEdges(Cluster cluster)
    {
        ...
        if (cluster.DoorPoints.Count == 0) return;
        ...
        foreach (var doorPoint in cluster.DoorPoints)
        {
            ...
            for (var i = 0; i < cluster.IntraEdges.Count; i++)
            {
                ...
                if (edge.Path[0] ==
                    doorPoint.Position) //We may have already found the intraedge with one of the other shortcuts.
            ...
            if (addUp) AddIntraEdge(doorPoint, cluster, Direction.Up);

            if (addRight) AddIntraEdge(doorPoint, cluster, Direction.Right);

            if (addDown) AddIntraEdge(doorPoint, cluster, Direction.Down);

            if (addLeft) AddIntraEdge(doorPoint, cluster, Direction.Left);
        }
    }

    public void AddIntraEdge(EdgePoint point, Cluster cluster, Direction adjacencyDirection)
    {
        if (point.AdjacencyDirection ==
            adjacencyDirection) //Don't waste time finding the shortest path, this has to be it.
        {
        ...
        foreach (var doorPoint in cluster.DoorPoints)
        {
            if (doorPoint.AdjacencyDirection !=
                adjacencyDirection) //We only want IntraEdges in the direction that we're currently trying to add them in the direction of
                continue;

            for (var i = 0; i < cluster.IntraEdges.Count; i++)
            {
                var edge = cluster.IntraEdges[i];
                if (edge.Path[0] != doorPoint.Position ||
                    edge.Path.Last() !=
                    point.Position) //We might have already found this path in the other direction and it is necessarily the shortest path by definition of how we found it.
                    continue;
